...
const readyToRecommend = ref(false)

const messages = ref([
  {
    role: 'assistant',
    content:
      '안녕하세요. 아이와 살기 좋은 서울 자치구를 찾아드릴게요. 아이의 나이를 알려주세요.'
  }
])
  ...
  try {
    ...
    if (data.extracted_profile) {
      profile.value = {
        ...
        selected_categories:
          data.extracted_profile.selected_categories ??
          profile.value.selected_categories ??
          []
      }
    }

    readyToRecommend.value = data.ready_to_recommend ?? false

    console.log('챗봇 응답:', data)
    console.log('저장된 프로필:', profile.value)
    console.log('추천 가능 여부:', readyToRecommend.value)
  } catch (error) {
...
function saveRecommendationProfile() {
  ...
  saveProfile({
    ...
    priorities: categories.map((category) => ({
      category,
      weight: category === profile.value.top_priority ? 5 : 3
    })),
